...
release-binary: ## Build release binaries (used to build a final container image).
	@go build -o /go/bin/dex -tags netgo -v -ldflags $(LD_FLAGS) $(REPO_PATH)/cmd/dex
	@go build -o /go/bin/docker-entrypoint -tags netgo -v -ldflags $(LD_FLAGS) $(REPO_PATH)/cmd/docker-entrypoint
